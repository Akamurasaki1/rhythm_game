...
/// - SheetNote 型と、10 個分のサンプル譜面を提供する SampleData
public struct notes: Identifiable {
    public let id: UUID
...
public enum SampleData {
    public static let samples: [[SheetNote]] = makeSamples()

    // 10 個のサンプル譜面をプログラム生成して返す
    private static func makeSamples() -> [[SheetNote]] {
        var sets: [[SheetNote]] = []
        for i in 0..<10 {
            var arr: [SheetNote] = []
            ...
            for j in 0..<6 {
                ...
                arr.append(SheetNote(time: t, angleDegrees: angle, normalizedPosition: <#Position#>))
            }
            sets.append(arr)
        }
        return sets
