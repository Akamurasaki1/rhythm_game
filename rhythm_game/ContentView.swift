...
struct ContentView: View {
    ...
    // --- existing helper: try to find audio in bundle (unchanged) ---
    ...
    // --- existing bundled-sheets loader (unchanged) ---
    ...
    var body: some View {
        GeometryReader { geo in
            ...
            // file importer: 以前の実装に合わせて JSON / audio を受け取れるように
            ...
            .onAppear {
                // 起動時に bundled-sheets 内の譜面を読み込んでラベルとして使う
                                ...
                                withAnimation {
                                    ...
                                    if !isPlaying {
                                        if sampleIndex >= sampleDataSets.count {
                                            ...
                                            notesToPlay = sampleDataSets[sampleIndex]
                                        }
                                    }
                                }
                            }
    ...
    func handleImportedFile(url: URL) {
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            do {
                ...
                // reload samples/UI on main thread: if a JSON was imported it will appear in documents;
                // if audio was imported it will be found by bundleURLForAudio fallback when playing bundled sheet.
        ...
        if selectedSampleIndex >= sampleDataSets.count {
            ...
            if bundledSheets.indices.contains(bidx) {
                ...
                if let audioName = sheetForOffset?.audioFilename {
                    ...
                    // documents fallback omitted here for simplicity; keep existing behavior minimal
                }
            }
        }
